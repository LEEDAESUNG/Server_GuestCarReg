					...
					$.ajax({
							...
							success: function (response) {
									if(response.result == 1){
											alert('정보수정 완료됐습니다. 로그인 후 즉시 이용할 수 있습니다');
											location.replace('/login.php');
									} else if(response.result == 2){
											alert('가입신청 완료됐니다. 관리실로 [사전방문예약] 승인요청 하세요');
									} else if(response.result == 0){
											alert('사용할 수 없는 아이디 입니다. 관리실로 문의바랍니다(에러코드:PI_E001)');
											//.id.setfocus(); //에러
									} else if(response.result == -1){
											alert('가입신청 처리중 에러가 발생했습니다. 잠시후 재시도하세요(에러코드:PI_E002)');
									} else {
											alert('가입신청 처리중 에러가 발생했습니다. 잠시후 재시도하세요(에러코드:PI_E003)');
									}
							},
							error: function(jqXHR, textStatus, errorThrown) {
									alert("ajax error : " + textStatus + "\n" + errorThrown);
									alert('정의되지 않은 에러 발생. 동일현상 반복적으로 발생시 관리실 문의바랍니다(PI_E999)');
							}
					}); // $.ajax
			}); // $('#join-submit').click(function(e)
	}); // $(document).ready(function()
...
<body>
	<nav class="navbar navbar-light bg-light">
	    ...
	    Parking System
					...
					<form role="form">
			    <table style="font-size:18px; width:100%; font-family:'나눔고딕'" >

							<tr style="width:100%;"></tr>
							<tr style="width:100%;">
								<!-- <td colspan='2' align='center' style="font-size:30px" >사전방문 예약시스템<br>(가입신청)</td> -->
								<td colspan='2' align='center' style='font-size:30px '>[사전방문 예약시스템]</td>
							</tr>
	                ...
	                <th><input style="background-color:#e6e6e6; line-height:1.8; " type="text" numberOnly name="ho" id="inputHo" placeholder="*호수" onfocus="this.placeholder = ''" onblur="if(this.placeholder=='')this.placeholder='*호수'; NumeircCheckFunc(this);"></th>
							...
							<tr>
								...
								?>
									<th><input readonly style="background-color:#e6e6e6; line-height:1.8; " type="text" name="id" id="inputID" placeholder=<?php echo $LoginID ?> value=<?php echo $LoginID ?>></th>
			        </tr>
...
-->
			        ...
			        <tr>
			            ...
			            <!--<td colspan='2' align='center'><input type="submit" value="가입신청"></td>-->
                            ...
                            갱&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;신&nbsp;&nbsp;<i class="fa fa-check spaceLeft"></i></button> <!--fa-check:awesome font -->
							...
							<tr>
								...
								<th align='left'>
									...
									<span style='color:orange; font-weight:bold; font-size:0.8em; '>해야만 이용가능합니다 </span><br> -->

									<span style='color:orange; font-weight:bold; font-size:0.8em; '>1)관리실에 등록된 차량번호 입력시</span><br>
									<span style='color:orange; font-weight:bold; font-size:0.8em; '>즉시 이용가능합니다. </span><br>
									<span style='color:orange; font-weight:bold; font-size:0.8em; '>2)미등록 차량번호 입력시</span><br>
									<span style='color:orange; font-weight:bold; font-size:0.8em; '>관리실 사용승인 후 이용가능합니다 </span><br>
								</th>
							</tr>
			    </table>
